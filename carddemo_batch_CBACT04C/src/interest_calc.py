...
def join_interest_rates(
    ...
    primary_rate_df = discgrp_df.select(
    ...
    result = joined_with_default.withColumn(
        "effective_int_rate",
...
def compute_monthly_interest(tcatbal_with_rate_df: DataFrame) -> DataFrame:
    ...
    # BR-1: monthly_interest = (tran_cat_bal * dis_int_rate) / 1200
    # All operands must be Decimal; result cast to DECIMAL(11,2) to match COBOL S9(9)V99
    interest_df = non_zero_rate_df.withColumn(
        "monthly_interest",
        (
            F.col("tran_cat_bal").cast(DecimalType(11, 2))
            * F.col("effective_int_rate").cast(DecimalType(7, 4))
            / F.lit(Decimal(str(INTEREST_DIVISOR)))
